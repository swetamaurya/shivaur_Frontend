...
createEstimateForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        const invoiceDate = document.getElementById('estimateDate').value;
        const dueDate = document.getElementById('expiryDate').value;
        ...
        const response = await fetch(`${invoice_API}/post`, {
            ...
            body: JSON.stringify({
                email, clientAddress, billingAddress, invoiceDate, dueDate, 
                details, tax, total, GrandTotal, client, project, taxType, discount, 
                otherInfo
            })
        })
